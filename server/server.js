...
let addproductschema = require("./addproductmodel");
let PurchaseModel = require("./purchaseModel");
...
let purchasePaymentsSchema = require("./purchasepaymentsModel");
...
app.post("/addproducts",upload.array('img'), async(req,res)=>{
    try{
        // console.log(req.body);
        // console.log(req.files)

     const { productname, producttype, productprice, items, productdesc} = req.body; 
     let images;
         if( req.body.userimg === 'null'){
             images = null
         }
       ...
       let savedata= new addproductschema({ 
...
app.get("/allproductlist",async(req,res)=>{
    try{
        return res.status(200).json(await addproductschema.find())
    }
    catch(err){
        console.log(err)
        return res.status(500).send("internal server error...")
...
app.delete("/allproductlist/:id",async(req,res)=>{
    try{
        await addproductschema.findByIdAndDelete(req.params.id)
...
app.put("/updateproducts/:id",upload.array('img'),async(req,res)=>{
    try{
        ...
        let itemss;
         ...
         if( req.body.userimg === 'null'){
          ...
          await addproductschema.findByIdAndUpdate(userId, { 
            productname:productname, 
            producttype :producttype,
            productprice :productprice,
            items : itemss,
         ...
         .then(updatedUser => {
            if (!updatedUser) { 
             ...
             return res.send({status: 200, message: "Product Updated successsfull..."});
           })
...
app.post("/purchasetransaction", async(req,res)=>{
    try{
        ...
        //     let mapp = rows.map(list=>list.productname)[i];
        //     console.log("first", rows.map(list=>list.productname), rows.length, i , mapp)
        //     let exist = await  addproductschema.findOne({productname:mapp});
        ...
        // }
        // else{
        //     let qua = rows.map(list=>list.quantity)[i];
        //     console.log("qua", qua) 

        //     let totalQuantity = exist.items + qua; 
...
//purchaseForm get route

app.get("/getpurchasetransaction", async(req,res)=>{
    try{       
        return res.status(200).json(await PurchaseModel.find())
...
app.delete("/deletepurchasetransaction/:id", async(req,res)=>{
    try{
        await PurchaseModel.findByIdAndDelete(req.params.id);

        return res.json(await PurchaseModel.find())
    }
    catch(err){
        console.log(err)
        return res.status(500).send("internal server error...")
...
app.post("/purchasepayments", async(req,res)=>{
    try{
        const {dateOfPayment, purchaseno , vendorname , totalAmount , receiveAmount, PayAmount, paymentmethod , paymentstatus ,holdername, cardnumber, TranscationID } = req.body;

        const receiveAmounts =Number(receiveAmount) + Number(PayAmount); 

        let purchasedata = new purchasePaymentsSchema({dateOfPayment, purchaseno , vendorname , totalAmount ,receiveAmount : receiveAmounts, PayAmount, paymentmethod , paymentstatus ,holdername, cardnumber, TranscationID});
        ...
        if(paymentmethod === "Google Pay"){
            await PurchaseModel.findByIdAndUpdate(exist._id, {paymentmethod : paymentmethod, paymentstatus : paymentstatus, holdername : holdername, cardnumber :TranscationID, receiveAmount :receiveAmounts }, {new: true})
            .then(transcation => {
                if (!transcation) {
                res.send({ status: 404, message: 'Payment Transcation Failed ...' });
                } else {
        ...
        if(paymentmethod === "Credit Card"){
            await PurchaseModel.findByIdAndUpdate(exist._id, {paymentmethod : paymentmethod, paymentstatus : paymentstatus, holdername : holdername, cardnumber :cardnumber, receiveAmount :receiveAmounts }, {new: true})
            .then(transcation => {
                if (!transcation) {
                res.send({ status: 404, message: 'Payment Transcation Failed ...' });
                } else {
...
app.get("/getpurchasepayment", async(req,res)=>{
    try{
        res.status(200).json(await purchasePaymentsSchema.find());
    }
    catch(err){
        console.log(err)
         return res.status(500).send("internal server error...")
...
app.post("/captureImg", captureimgUpload.single("photo"),async(req,res)=>{
    try{
        ...
        console.log(err)
        res.send({status:500, message:"not send"})
    }
})
