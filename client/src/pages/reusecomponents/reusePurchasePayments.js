...
const ReusePurchasePayments = (params) => {
    ...
    const handleClick =(purchaseData)=>{
        navigate('/purchasepaymentdetails', { state: purchaseData});
    }
  ...
  const [checkStatus, setCheckStatus] = useState(null);
    ...
    const formik = useFormik({
        ...
        validationSchema : Yup.object({
            ...
            purchaseno : Yup.string().required("Select purchase No"),
        ...
        onSubmit : async(values, {resetForm}) => {
           ...
           await axios.post(`${API_BASE_URL}/purchasepayments`, values).then(res=>alert(res.data.message)).catch(err=>console.log(err));

           
    // pass salesPayment date to sales payment details page 
   
         navigate('/purchasepaymentdetails', { state: values});
    }
    ...
    const handlepurchaseID = (e)=> {
        formik.setFieldValue("purchaseno", e.target.value);

        //find index
        let index = purchase.findIndex(list=>list.purchaseno === e.target.value);
        formik.setFieldValue("vendorname", purchase[index].vendorname);
        formik.setFieldValue("totalAmount", purchase[index].totalAmount);
        formik.setFieldValue("receiveAmount", purchase[index].receiveAmount);

        //Check Status
        setCheckStatus( purchase[index].paymentstatus)
    }
                ...
                <div className="container-fluid">
                    ...
                    <div className="row">
                        <div className="col-12">
                            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 className="mb-sm-0">Purchase Payments</h4>

                                <div className="page-title-right">
                                    <ol className="breadcrumb m-0">
                                        <li className="breadcrumb-item"><a href="#a">Payments</a></li>
                                        <li className="breadcrumb-item active">Purchase Payments</li>
                                    </ol>
                                </div>
                        ...
                        <div className="col-sm-auto ms-auto">
                           <div className="d-flex gap-3">
                            <div className="search-box">
                                <input type="text" className="form-control" id="searchMemberList" placeholder="Search for Company" name='search' onChange={e=>setSearch(e.target.value)} />
                    ...
                    <div className="row">
                                                ...
                                                <div className="card-body">
                                                    <div className="table-responsive table-card">
                                                        <table className="table table-hover table-nowrap align-middle mb-0">
                                                            ...
                                                            <tbody>
                                                                {currentData.length > 0 ? currentData.filter(list=>list.vendorname.toLowerCase().startsWith(search.toLocaleLowerCase()) || list.purchaseno.startsWith(search)).map((list ,id)=>
                                                                <tr key={id}>
                                                ...
                                                <div className="card-body">
                                                    <div className="table-responsive table-card">
                                                        <table className="table table-hover table-nowrap align-middle mb-0">
                                                            ...
                                                            <tbody>
                                                                {purchase.filter(list=>list.paymentstatus === "Paid").length >0 ?  purchase.filter(list=>list.vendorname.toLowerCase().startsWith(search.toLocaleLowerCase())  && list.paymentstatus === "Paid").map((data, id)=>
                                                               <tr key={id}>
                                                ...
                                                <div className="card-body">
                                                    <div className="table-responsive table-card">
                                                        <table className="table table-hover table-nowrap align-middle mb-0">
                                                            ...
                                                            <tbody>
                                                                {purchase.filter(list=>list.paymentstatus === "Unpaid").length >0 ?  purchase.filter(list=>list.vendorname.toLowerCase().startsWith(search.toLocaleLowerCase())  && list.paymentstatus === "Unpaid").map((data, id)=>
                                                               <tr key={id}>
                                                               <td>{data.purchaseno}</td>
                                                              <td>
                                                                  <p className="text-body align-middle fw-medium">{data.vendorname}</p>
                                                ...
                                                <div className="card-body">
                                                    <div className="table-responsive table-card">
                                                        <table className="table table-hover table-nowrap align-middle mb-0">
                                                            ...
                                                            <tbody>
                                                                {purchase.filter(list=>list.paymentstatus === "Pending").length >0 ?  purchase.filter(list=>list.vendorname.toLowerCase().startsWith(search.toLocaleLowerCase())  && list.paymentstatus === "Pending").map((data, id)=>
                                                               <tr key={id}>
                                                               <td>{data.purchaseno}</td>
                                                              <td>
                                                                  <p className="text-body align-middle fw-medium">{data.vendorname}</p>
                        ...
                        <div className="col-md-5">
                            <p className="mb-0 text-muted">Showing <b>1</b> to <b>15</b> of <b>{purchase.length}</b> results</p>
                        </div>
                        ...
                        <div className="row">
                            <div className="col-lg-12">
                                ...
                                <div className="mb-3">
                                    ...
                                    <select type="text" className="form-control" id="purchaseno" name='purchaseno' onChange={handlepurchaseID} > 
                                    <option> --- Select purchase ID ---</option>
                                    {purchase.map((list, id)=> <option key={id} value={list.purchaseno}>{list.purchaseno}</option>)}
                                    </select>
                                ...
                                <div className="mb-3">
                                    ...
                                    <input type="text" className="form-control" id="PayAmount" name='PayAmount'{...formik.getFieldProps("PayAmount")} placeholder={`Balance To Be Paid : ${formik.values.receiveAmount ? Number(formik.values.totalAmount) - Number(formik.values.receiveAmount) : 0}`}/>
                                ...
                                {checkStatus === "Paid" &&
                                <div>
                                    <p  style={{textAlign:"center", fontSize:"1.5rem"}}><mark>Your Payment Was Done...</mark></p>
                                </div>
                                }
                            </div>
                        </div>
...
export default ReusePurchasePayments