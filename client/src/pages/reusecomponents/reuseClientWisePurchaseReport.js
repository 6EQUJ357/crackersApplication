...
const ReuseClientPurchseReport = (params) => {
   ...
   const [searchproduct, setSearchproduct] = useState("")
    ...
    let fillVendorName = purchase.map(list=>list.vendorname);

    let uniqueArrayName = fillVendorName.filter((value, index, self)=>{
    ...
    const handleFilter = ()=>{
        if(!searchproduct){
            ...
            setFilter(purchase.filter(list => list.vendorname.toLowerCase().startsWith(searchproduct.toLowerCase())))
        }
    }
                        ...
                        <div className="col-sm-4">
                            <div className="d-flex">
                                ...
                                <select className="form-control"  name="searchvendor" value={searchproduct} onChange={(e)=>setSearchproduct(e.target.value)}>
                                    ...
                                    {uniqueArrayName.length > 0 && uniqueArrayName.map((list,id)=><option key={id} value={list}>{list}</option>)}
                                </select>
...
export default ReuseClientPurchseReport